...
description: Deploy, activate, inspect, and troubleshoot StellaGate-UI on a user's VPS through an AI agent workflow. Use when a user asks Codex, Claude Code, or another agent to install StellaGate-UI, configure the official authorization endpoint, activate with an invite code, verify panel/Xray/subscription health, explain StellaGate activation errors, or check authorization API contract compatibility. Do not use for building a VPN marketplace, payment system, user registration, node selling, arbitrary remote shell, or storing VPS root credentials.
---
...
- Prefer HTTPS authorization endpoint URLs. Localhost HTTP is only acceptable for local development.
...
- For authorization API compatibility, payloads, or contract drift, read `references/cloud-api-contract.md`.
...
3. Generate the installer command with `scripts/build_install_command.py`. Default authorization endpoint URL is `https://stellagate.simuse.uk`.
...
7. Verify:
   ...
   - `/etc/x-ui/stellagate-cloud.json` contains the intended authorization endpoint URL.
...
- Activation state and authorization endpoint URL
- Xray/service state
...
For failures, say exactly which stage failed: access, environment, installer download, service start, authorization endpoint configuration, invite activation, node creation, subscription generation, or Xray health. Then map the StellaGate error code to plain language with `references/error-codes.md`.
